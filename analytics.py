...
def extractLinkList(sourceList):
	...
	with open(os.path.join(cwl,'links.dump'),'w') as f:
		pickle.dump(src, f)
...
def scrapeData(linkSource):
	...
	with open(os.path.join(cwl,'ifirm.list'),'w') as f:
...
if __name__=="__main__":
	...
	if os.path.isfile(os.path.join(cwl,'links.dump')) == False:
		linkList = extractLinkList(data)
	else:
		with open(os.path.join(cwl,'links.dump'),'r') as f:
			linkList = pickle.load(f)
	DATA = scrapeData(linkList)

	with open(os.path.join(cwl,'fData.dump'),'w') as f:
		pickle.dump(DATA,f)

	TS_DATA = parseBhavData(bhavDataLoc)

	with open(os.path.join(cwl,'tData.dump'),'w') as f:
